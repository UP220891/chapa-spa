...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    // Insertar usuario en T_Auth como cliente
...
router.post('/register-empleado', verificarToken, async (req, res) => {
  ...
  if (!req.user || req.user.tipo_usuario !== 'empleado' || req.user.rol !== 'admin') {
    return res.status(403).json({ mensaje: 'Solo el admin puede crear empleados' });
  }
  const { nombre, email, password, telefono, fechaNacimiento, rol } = req.body;
  if (!nombre || !email || !password || !telefono || !fechaNacimiento || !rol) {
    return res.status(400).json({ mensaje: 'Todos los campos son obligatorios' });
  }
  try {
    const pool = await poolPromise;
    ...
    // Insertar usuario en T_Auth como empleado
    const hash = await bcrypt.hash(password, 10);
    await pool.request()
      .input('email', sql.NVarChar(50), email)
      .input('password', sql.NVarChar(255), hash)
